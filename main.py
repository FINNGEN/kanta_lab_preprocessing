...
def all_filters(df,args):
    b = True if "Puuttuu" in df else False
    if b:print(f"before:{df.laboratoriotutkimusnimikeid}")
    df.pipe(filter_minimal,args)
    if b:print(f"after:{df.laboratoriotutkimusnimikeid}")
    return df
...
def main(args):
    ...
    for i,chunk in enumerate(chunk_reader(args.raw_data,args.chunk_size)):
        ...
        chunksize = len(df_split[0])
        results = pool.imap(multi_func, df_split,chunksize=chunksize)
        ...
        if i ==0:
            ...
            logger.info(f"sub chunk size:{chunksize}")
        else:
            size += len(df)
