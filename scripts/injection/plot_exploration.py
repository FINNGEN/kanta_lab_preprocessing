...
If plot_name contains a PLOT_CATEGORY column it is used to color points;
otherwise all points are drawn in a single colour.
"""
...
_COLORS = {"has OMOP": "steelblue", "no OMOP": "tomato"}
...
def make_scatter_plot(plot_name, output=DEFAULT_OUTPUT):
    ...
    if "OMOP_CONCEPT_ID" in plot_name.columns:
        groups = {
            "has OMOP": plot_name[plot_name["OMOP_CONCEPT_ID"].notna()],
            "no OMOP":  plot_name[plot_name["OMOP_CONCEPT_ID"].isna()],
        }
    else:
        groups = {"all": plot_name}
        _COLORS["all"] = "steelblue"
    ...
    for label, sub in groups.items():
        prev = sub["top_prevalence"].dropna().values
        if len(prev) < 2:
            continue
        ys  = np.linspace(prev.min(), prev.max(), 400)
        kde = gaussian_kde(prev, bw_method="scott")
        ax_kde.fill_betweenx(ys, kde(ys), alpha=0.2, color=_COLORS[label])
        ax_kde.plot(kde(ys), ys, color=_COLORS[label], lw=1.2)
