...
from kanta.engine.filters import filter_minimal, fix_unit
...
def run_all(
    df: pd.DataFrame, errors: ErrorSink, abbr_changes: AbbrSink, unit_changes: UnitSink
) -> pd.DataFrame:
    return (
        df.pipe(rename_cols)
        .pipe(snapshot_source_cols)
        .pipe(filter_minimal.run, errors, abbr_changes)
        .pipe(fix_unit.run, unit_changes)
